...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  ...
  vp[0:2,:]       occupied x,y positions from range sensor (in physical unit) (world frame) 
  xs,ys           physical x,y,positions you want to evaluate "correlation" (vibration)
  OUTPUT 
  ...
  for jy in range(0,nys):
    y1 = vp[1,:] + ys[jy] # 1 x 1081
    iy = np.int16(np.round((y1-ymin)/yresolution))
    for jx in range(0,nxs):
      x1 = vp[0,:] + xs[jx] # 1 x 1081
